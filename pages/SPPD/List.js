...
class SppdRequest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoading: true,
  ...
  generateList = (items) => {
    ...
    if (list.type == '') {
      ...
      if (list.nama_kasus == 'Surat Tugas') {
        tipeSppd = arrTipeSppd[list.type - 1].jenis;
      } else {
        tipeSppd = '-';
      }
    }
      ...
      <TouchableOpacity>
        <List>
          ...
          <ListItem>
            ...
            <Right>
              <Text>
                {list.tgl_mulai ?? '-'} s.d {list.tgl_selesai ?? '-'}
              </Text>
            </Right>
          </ListItem>
...
export default connect(mapStateToProps)(SppdRequest);
