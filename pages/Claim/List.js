...
class ClaimRequestList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoading: true,
  ...
  viewBerkas = (target) => {
    this.props.navigation.push('PreviewAttachment', {url: target});
  };
  ...
  generateList = (items) => {
    ...
    let jml_klaim =
      typeof list.jumlah_klaim == 'undefined'
        ? '0'
        : list.jumlah_klaim.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.');
          ...
          <ListItem>
            <Left style={style.leftSide}>
              <Image
                ...
                style={styles.imageInputLabel}
              />
              <Text>Tanggal Berkas</Text>
            </Left>
            <Right>
              <Text>{list.tgl_klaim ?? '-'}</Text>
            </Right>
          </ListItem>
          <ListItem>
            <Left style={style.leftSide}>
              <Text>Jumlah Klaim</Text>
            </Left>
            <Right>
              <Text>{jml_klaim ?? '-'}</Text>
            </Right>
          ...
          <ListItem style={{paddingBottom: 20}}>
            <Left style={{flexDirection: 'column'}}>
              <View style={[style.leftSide, {width: '100%'}]}>
                {/* <Image source={require('assets/purposetype.png')} style={styles.imageInputLabel} /> */}
                <Icon name="attach" />
                <Text>Berkas</Text>
              </View>
              {!list.berkas ? (
                <Text
                  ...
                  onPress={() => this.viewBerkas(list.berkas)}>
                  Lihat berkas
                </Text>
              ) : (
                <Text
              ...
              {/* <Text style={{width: '100%', top: 10, color:'#1565c0', left:20}} onPress={() => this.viewBerkas(e.berkas)}>{(e.berkas!=' ')?'Lihat berkas':'Tidak ada lampiran'}</Text> */}
            </Left>
            <Right>
              <Button
                success
...
export default connect(mapStateToProps)(ClaimRequestList);
